...
- **Resource estimate.** Using a Voronoi pay-zone-only volumetric (the standard placer-mining method, also what the 2024 independent analysis used), our 24-hole subset gives **9,631 fine ounces of gold** vs. their published 10,056 — agreement within 4%.
...
A three-dimensional view of the cluster, rendered with PyVista. Each drill collar is a vertical stick descending from the surface (z=0) to its bedrock contact, segmented at every captured per-2-ft interval and colored by that interval's grade on the same standard placer grade-tier scheme used in the 2D figures (white < 0.005, light blue 0.005-0.010, yellow 0.010-0.020, orange 0.020-0.050, red 0.050-0.100, magenta > 0.100 oz/cu yd). The shaded surface beneath the sticks is the bedrock topography — a thin-plate-spline RBF fit on the 24 holes' bedrock depths (3 NBR holes carry their KNN-IDW imputed values; see [§6.13 Bedrock imputation review](#bedrock-imputation-review)).
...
Two prior independent analyses have estimated the gold resource on the broader Bear Cub / Dry Gulch property using the same drill-log archive: a 2021 per-interval re-transcription of the logs, and a 2024 independent mining-engineer polygon-volumetric block model on the property scale. They had hundreds of holes; we have a 24-hole Murray subset. We reproduce their methodology end-to-end on our subset, both to validate the pipeline and to produce a comparable resource estimate.
...
Across 691 intervals captured in the corpus, **74 intervals** exceed the standard "marginal" threshold of 0.005 oz/cu yd. The single highest-graded 2-foot interval in the dataset is in `L2 H4` at the bedrock contact, at 1.34 oz/cu yd — a substantial hit, though that figure is high enough to warrant manual verification of the underlying mg reading.

### Per-hole grade profiles

Each hole's grade as a function of depth, plotted as a horizontal bar chart with depth increasing downward (the convention in geology). Colors follow the standard placer grade-tier classification: white below 0.005 oz/cu yd, light blue at 0.005-0.010, yellow at 0.010-0.020, orange at 0.020-0.050, red at 0.050-0.100, magenta above 0.100. All 24 subplots share a common x-axis (0 → corpus max), so a 0.005 bar in one panel reads visually the same as a 0.005 bar anywhere else; each subplot's title prints that hole's peak so per-hole magnitude stays legible.

```{python}
#| label: grade-profiles
#| code-fold: true
#| fig-cap: "Grade-vs-depth profiles for all 24 holes. Shared x-axis 0 → corpus max (~0.6 oz/cu yd). Each subplot's title shows that hole's peak grade. Colors follow the standard placer grade-tier classification (white < 0.005, light blue 0.005-0.010, yellow 0.010-0.020, orange 0.020-0.050, red 0.050-0.100, magenta > 0.100). Brown dashed line = bedrock contact. Holes sorted west→east."
...
#| label: plan-map
#| code-fold: true
#| fig-cap: "Left: drill-hole positions on a local-feet grid, colored by surface-to-bedrock grade using the polygon-block classification scheme. Right: bedrock-depth surface (RBF interpolation) — deepest in the SE, shallowest in the NW. Bear Cub MS 1178 outline in red."
Image(str(REPO / 'data' / 'derived' / 'bear_cub_resource' / 'fig_plan_map.png'))
```

### Surface-to-bedrock grade map

A reproduction of the published independent grade-adjusted-to-bottom-of-pay figure, on our subset. For every point in the property polygon, we compute the depth-averaged grade across the full drilled column (surface to bedrock) using the nearest hole. This is what you'd recover per cubic yard if you dredged the entire column down to bedrock at any point.
...
Our 24-hole map shows the same broad pattern the 2024 published analysis identifies on the full property: higher grade in the SW and south-central area, lower grade to the NE. The Murray cluster doesn't cover the full claim — adding the rest of the archive would extend the surface across the whole patent.
...
The pay-zone-only point estimate is the right number to compare against the 2024 published independent analysis. Their published 10,056 oz sits outside the parameter-noise credible interval (see Monte Carlo below), so the gap is methodological (sliding-window pay-zone definition vs the published uniform-thickness assumption), not measurement noise.
...
- **Sliding-window vs the 2024 analysis's "uniform 8-ft pay layer".** Different pay-zone definitions, both internally consistent, produce different totals. Our pay-zone-only number being 59% of the 2024 published number reflects this choice.
...
The same Voronoi tessellation, colored by each cell's pay-zone average grade. This is the closest analog to the published independent "Grade Map adjusted to Bottom of Pay" — it shows where on the property the gold-bearing layer is richest.

```{python}
#| label: pay-zone-map
#| code-fold: true
#| fig-cap: "Voronoi cells colored by pay-zone average grade. The southern + south-central holes show the highest concentrations, consistent with the published independent narrative about the dredge cuts."
...
#| code-fold: true
#| fig-cap: "Same aerial overlay with the pay-zone Voronoi cells filled in (polygon-block grade map adjusted to BOP). Highest-grade cells visible along the southern boundary."
...
Three of our holes appear in either of the two prior independent published analyses (2021, 2024) — or both. Comparing per-hole grades:
...
**Where we agree.** Hole `L6700 H6760` comes out at 0.045 oz/cu yd in our pipeline at the 40-48 ft pay zone versus the 2024 published 0.050 — within 10%. Same source data, two independent transcriptions (ours via OCR, theirs manual), independent calculations, agreement at the ten-percent level. This is the kind of validation that would be impossible with a single transcription pass.

**Where we differ.** Hole `L7100 H7156` has a Hammon Prospect form variant where the operator only recorded total weights at the sample level (a 14-foot interval covering the whole pay zone) on the back of the page, with per-2-foot intervals showing only "VF" / "F" letter codes. The 2021 analysis identified a tighter pay zone of 6-14 ft (8 feet) within that 14-foot sample, getting a grade of 0.05 oz/cu yd by attributing all 115 mg of recovered gold to those 8 feet. Our pipeline's color-weighted distribution spreads the 115 mg across the full 0-14 ft sample range, getting 0.030. Both are math-correct; they're different pay-zone definitions. This is the methodological choice that the resource-analysis comparison surfaces.
...
2. **More samples.** The 2024 full-property analysis has hundreds of holes. With n ≈ 100+, even a vanilla spatial GP would reasonably learn the gradient.
...
4. **`L6900 H6952`** — pay-zone grade 0.111, which is high enough to deserve a sanity check. The 2021 published analysis notes a "wide gold anomaly" on this hole, so the high grade is consistent with that prior interpretation, but a manual mg verification is worth doing.
...
- **Validation against an independent archive.** The cross-validation with the 2021 and 2024 prior analyses uses the same source data we did. A stronger test would be running the pipeline on a separately-archived dataset (e.g., from the Janin Huntington archive, the next batch on the Nome Placer Fields property) and comparing to whatever ground-truth exists there.
...
- **Convention C lumping under-counts grade.** Hammon Prospect Drilling Logs (and a few Hammon Field variants) record gold weight only at the sample level on the back of the sheet — the front-page `Weight of gold` column has text codes like `VF`, `F`, `-` instead of numbers. If you ingest these naively, you get zero grade for the hole even though substantial mg is recorded. Discovered when our pipeline returned 0 oz/yd³ for `L7100 H7156` while the 2021 published number was 0.053. Fix: form-aware re-OCR prompt that reads the back-page sample-totals and treats them as authoritative when the front-page mg column is blank, then distributes sample-level mg across constituent intervals proportionally to color counts (the operator's visual gold-density signal).
...
## Per-hole comparison detail (vs. the 2021 and 2024 prior published analyses)
...
both = df[(df['y2021_grade_lo'].notna()) & (df['ours_pay_zone_grade_peak'].notna())]
both[['hv_id','bear_cub_log','y2021_pay_zone_ft','y2021_grade_lo','y2021_grade_hi',
      'ours_avg_over_y2021_zone','ours_peak_in_y2021_zone','comparison_flag']]
```

The `comparison_flag` column compares our depth-weighted average across the 2021 published pay zone to that analysis's stated grade. The 2021 grades are published to one significant figure (e.g., 0.01, 0.02, 0.04-0.05). When ours rounds to that stated value, that's a match even if the unrounded difference is 20-40%. The genuine disagreements after applying that lens:

- **HV-7156** (-40%): the 2021 analysis uses a tighter 6-14 ft zone within the operator's 0-14 ft sample 1, concentrating 115 mg into 8 ft instead of our 14 ft. Both math-correct, different definitions.
- **HV-6960** (+68%): the 2021 analysis says "8-18" but the operator's sample 1 is 0-18; over the actual 0-18 range our value is within 8% of theirs. That analysis appears to have mis-recorded the depth label.
