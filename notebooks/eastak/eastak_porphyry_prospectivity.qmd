...
5. **External validation is out of scope for Eastern Alaska.** The public-data regime for post-training porphyry drill results in this AOI is essentially N=1 — too thin for portfolio-grade validation. The distribution-based blind test against 366 post-2015 drill collars is in the **[BC Golden Triangle v2 track](../bcgt/bcgt_porphyry_prospectivity.qmd)**; Eastern Alaska establishes the methodology and the bias diagnosis that BCGT then quantifies in distribution.
...
## External validation — why it lives in the v2 BCGT track
...
For Eastern Alaska the public-data regime is **essentially N=1**.
Post-training porphyry drill disclosures in this AOI are limited to
a single named collar with approximate coordinates (visually digitized
from a press-release plan map, ~±5 km uncertainty). One hole is not a
portfolio-grade validation signal — it's an anecdote, not a
distribution.

**We ran the distribution-based blind test on BC Golden Triangle
instead.** The BCGS GeoFile 2025-11 Assessment Report Drillhole Database
publishes 366 post-2015 drill collars in our v2 AOI with per-hole
element maxima — 154 intersected (meeting a threshold on Cu / Mo / Au /
Ag) and 137 drilled-negative. 241 of those 366 are in cells the training
labels have zero info about. The distribution-based test lives in
[`notebooks/bcgt/validation.qmd`](../bcgt/validation.qmd); the summary
