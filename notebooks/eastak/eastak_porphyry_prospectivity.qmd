...
**Result: P = 0.017, 64th percentile.** The model completely fails to flag the one cell we can score blind. Positives median is 0.85 — roughly 50× higher. This is a single data point, but the gap is too large to explain as noise.
...
`<el>_has_data` doesn't land in the top-15 importances. Kenorland P barely moves. The NaN-vs-value channel exists but is minor — the magnitude-within-explored channel dominates.
...
| Kenorland 23ETD062 blind | P=0.017 vs positives median 0.85 | **fails** |
