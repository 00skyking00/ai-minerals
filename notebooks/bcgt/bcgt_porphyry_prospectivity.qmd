...
2. **The blind-test gap v1 identified is now testable at distribution scale.** v1's external validation was N=1 (Kenorland 23ETD062). v2 uses BCGS GeoFile 2025-11 — 366 post-2015 drill collars in AOI, **154 intersected cells + 87 non-intersected cells that are entirely absent from the training-label set**. Those 241 "previously unknown" holes (per our training data) are the blind-test set.
...
5. **The blind-test result is sharper than v1's and roughly as damning.** RF no-count achieves 0.82 ROC-AUC on spatial CV. The two blind-test distributions (154 positives + 87 negatives) differ statistically (K-S p = 1e-4) but the Mann-Whitney test that positives score higher than negatives is NOT significant (p = 0.26). Blind positives median P = 0.87; blind *negatives* median P = 0.98 — **the drilled-nothing cells actually score higher than drilled-ore cells** because they cluster in already-explored terrain near training positives. At the top-5 % AOI cutoff, 40 % of blind positives and 63 % of blind negatives are flagged — the ordering that matters for real drill targeting doesn't reliably pick winners over losers.
...
v1's N=1 Kenorland test becomes v2's N=366 distribution. For each post-2015 drill collar, score the nearest model cell and bucket by whether (a) the hole intersected a threshold and (b) the cell was in the training label set.
...
- **A real distribution-based external validation** of a pseudo-supervised MPM pipeline — where v1 had one anecdote, v2 has 241 "previously-unknown" drill cells with known outcomes.
