---
title: "Intro — what's in the Tanacross data and why"
...
print(f"  bounding box: W={TANACROSS.min_lon}, S={TANACROSS.min_lat}, "
      f"E={TANACROSS.max_lon}, N={TANACROSS.max_lat}  (WGS84)")
print(f"Working projection: {WORKING_CRS}  (NAD83 / Alaska Albers)")
```

## 1. The exploration question

We have a 1° × 2° patch of eastern Alaska — the Tanacross 1:250,000
quadrangle — and we want to build a model that says "**this pixel is more
likely to host a porphyry copper deposit than that pixel**." That's mineral
...
ardf_path = DATA_RAW / "ardf" / "ardf_tc.gpkg"
ardf = gpd.read_file(ardf_path)

print(f"MRDS: {len(mrds)} occurrences in bbox")
print(f"ARDF: {len(ardf)} occurrences in Tanacross quadrangle (TC)")
```
...
The Tanacross quadrangle hosts three kinds of systems simultaneously:

- Cretaceous-age **porphyry Cu-Mo** (Taurus Cu-Mo-Au-Ag is the big one).
...
These ~18 sites are our training positives. Notice how small that is. Class
imbalance is a defining feature of mineral prospectivity mapping — out of
the millions of pixels in the AOI, only a few dozen are confirmed deposits.
Day 3 addresses this directly (pseudo-negative sampling + PU learning
comparison).
...
Almost 4,700 samples in our AOI — a surprisingly dense geochemical record
for a 200 × 110 km patch of Alaska. About ⅓ are stream-sediment samples
(sediment collected where creeks drain the landscape — each sample
*integrates* rock fingerprinted from the entire upstream catchment).
That's huge for exploration: one creek can tell you about square kilometers
of otherwise inaccessible terrain.
...
ax.set_title(f"AGDB4 sample locations in Tanacross — {len(agdb):,} samples")
ax.set_aspect("equal")
...
```{python}
#| label: s2-load
s2 = rioxarray.open_rasterio(
    DATA_RAW / "sentinel2" / "s2_median_tanacross.tif", masked=True
)
print(f"Sentinel-2 composite: shape={s2.shape}, CRS={s2.rio.crs}")
...
for mineral mapping, took the per-pixel **median over 20 low-cloud scenes
from July–August 2024**, and reprojected to 60 m resolution Alaska Albers.
...
ax.set_title("Sentinel-2 natural color — Tanacross, summer 2024")
ax.set_aspect("equal")
plt.tight_layout()
```
...
ax.set_title("Copernicus GLO-30 DEM — Tanacross")
ax.set_aspect("equal")
plt.tight_layout()
```
...
```{python}
#| label: geology
geo = gpd.read_file(DATA_RAW / "geology_ak" / "geology_tanacross.gpkg")
...
ax.set_title("Tanacross composite — DEM + stream-sediment Cu + porphyry labels")
ax.legend(loc="lower left")
ax.set_aspect("equal")
plt.tight_layout()
```
...
1. **The label problem is stark.** ~18 known porphyries out of millions of
   pixels. Random undersampling vs PU learning is a real choice.
...
3. **The data are messy but honest.** Legacy samples from four decades,
   ...
   harmonization.
4. **The quadrangle has structure.** Porphyry sites cluster geographically
   and on specific geologic units. A model that respects spatial
