...
raster and sample layers. If the cached parquet is older than the source
data we regenerate.
...
There are two ways to define "not a deposit":
...
1. **Pseudo-negatives**: random cells drawn from the pool of cells that are
   ...
   same lithology mix as the positives. The 5 km buffer keeps us from
   training against a pixel that's a neighbor of an unconfirmed deposit.
2. **PU learning**: treats unlabeled cells as genuinely unlabeled, not
   negative. The [validation notebook](validation.qmd) adds the PU comparison as a sensitivity check.

Stratifying by lithology carries the experiment: porphyry positives cluster
in late-Cretaceous intrusive rocks. If we sampled random negatives without
...
area flagged prospective, as we lower the score threshold. This is the
standard MPM metric under heavy class imbalance, where it reports capture
without flattering the model the way AUC can.
...
Reshape the predictions into the grid and save as a GeoTIFF for downstream
use in the RF+SHAP and validation notebooks.
...
- The AUC numbers are inflated by folds with only 1 test positive. The
  [RF+SHAP notebook](random_forest_and_shap.qmd) tightens this with a
  per-fold breakdown.
- Logistic regression is the floor, not the ceiling. The Random Forest
  + SHAP interpretation shows whether the model is actually learning
  geology vs. confounds.
- 10 Rainbow Ridge positives have NaN Sentinel-2 features. The model
  ...
  predictions toward population mean. The
  [validation notebook](validation.qmd) checks whether dropping those
  positives materially changes the picture.
- No sensitivity battery yet: strict-21a label choice, PU-learning
  comparison, and the no-geochem exploration-robust baseline live in the
  [validation notebook](validation.qmd). For the distribution-based
  external blind test on 366 post-2015 drill holes, see the
  [BC Golden Triangle validation notebook](../bcgt/validation.qmd) and
  [BCGT integrated report](../bcgt/bcgt_porphyry_prospectivity.qmd).
  The Eastern Alaska story is about the modeling pipeline + sensitivity
  pass; BCGT is where we validate against real post-training drill data.
