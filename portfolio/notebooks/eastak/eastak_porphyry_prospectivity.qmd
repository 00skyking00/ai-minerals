...
subtitle: "A pseudo-supervised Random Forest, with external validation that doesn't flatter the model"
author: "Sky King"
date: "2026-04"
format:
  html:
...
1. Built a full mineral-prospectivity pipeline over three USGS 1:250,000 quadrangles of eastern Alaska (~62,000 km², the Yukon-Tanana to Wrangellia porphyry belt) using only public data: USGS MRDS, ARDF, AGDB4 geochemistry, SGMC geology, NRCan 100 m aeromagnetic (RTF + 1VD), USGS Bouguer gravity, Sentinel-2, and Copernicus GLO-30 DEM.
...
- `docs/mvp-proposal.md`: the scoping document; what we built and why.

## Why critical minerals, and why now

The energy transition is a tripling of global copper demand and a >10× increase in lithium, nickel, and cobalt demand over 2020 baselines ([IEA Critical Minerals Outlook 2024](https://www.iea.org/reports/global-critical-minerals-outlook-2024)). New porphyry-Cu production at scale is a decade behind that demand curve, and the bottleneck is not processing. It is *discovery*. Greenfield ore-body discovery rates have been declining for thirty years even as exploration spend has climbed.

This writeup builds the kind of simple regional-scale model an AI-driven mineral-exploration stack starts from, and pressure-tests it the way a senior ML engineer should before handing it to a geology team. The interesting finding is where the pressure-test fails.
...
| Validation | (deferred to v2 BCGT, see note below) | n/a | n/a |

Everything is reprojected to **EPSG:3338 (NAD83 Alaska Albers, equal-area)** and aggregated to a 500 m regular grid. The glue layer is in [`features/assemble.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/features/assemble.py); most of the model-training code lives in [`model.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/model.py) and [`model_rf.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/model_rf.py).
...
The class-imbalance is severe: 56 positives in ~270,000 cells (≈0.02 %). Two ways to frame it without overstating what the labels support:
...
Without the counts the top features shift to `*_max_5km` and `*_mean_5km` of the classical pathfinder suite: Ag, Te, Mo, Cu, Sb, Au, Pb. The *values* of the geochem, not the density of the sampling. This is the textbook porphyry halo, and it is the signal that reflects mineralization rather than exploration effort.
...
Pseudo-negative sampling assumes cells far from any known occurrence are non-deposits. PU learning (Mordelet & Vert-style bagging, implemented in [`model_pu.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/model_pu.py)) refuses that assumption:
...
What this test tells us: under training-label metrics alone, the result
does not depend on geochem. Whether that translates to unseen-drill
generalization is a separate question. See the BCGT distribution-based
blind test for the distribution-level answer.

### 7.5 Success-rate curve on the no-count model
...
| External blind test | (not run here, see [BCGT](../bcgt/bcgt_porphyry_prospectivity.qmd)) | n/a |

## What this does and doesn't demonstrate

**What Eastern Alaska v1 does demonstrate:**
...
- A three-channel decomposition of the exploration bias that
  pseudo-supervised MPM admits, and a finding that feature-
...
- That the model would find unrecognized mineralization in unexplored
  ...
  answers. See the [BCGT integrated report](../bcgt/bcgt_porphyry_prospectivity.qmd).
...
**How a reviewer should read this:** everything in this document is
...
5. **Decision-theoretic framing.** Rather than "rank cells," ask "where is P(deposit) × value-of-information highest given current posterior uncertainty?" POMDP drill planning per Mern et al. and the Caers Mineral-X line. The rank is not the answer, the *next drill hole* is. **Built in v2** on the BCGT prior. See [`notebooks/bcgt/decision_planning.qmd`](../bcgt/decision_planning.qmd).
...
7. **Dark-data ingestion.** OCR / VLM-based structured extraction from legacy assessment reports, scanned drill logs, and historical assays. Tractable with modern document AI (Azure Document Intelligence, Textract, or VLMs). **Built as a Bear Cub pilot.** See [`notebooks/bear_cub/main.qmd`](https://johnsondevco.com/bearcub/notebooks/bear_cub/main.html) (24 family-archive logs, 1919-1955) and [`notebooks/bear_cub/mining_review.qmd`](https://johnsondevco.com/bearcub/notebooks/bear_cub/mining_review.html) for the placer-engineer review.
...
`pyproject.toml` pins Python 3.11+; `uv.lock` pins every transitive dependency. The notebook has no hidden state between cells; restart-and-run-all passes. Raw data is regenerable from the fetch modules in [`src/ai_minerals/data/`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/data/); nothing large is tracked in git.

## Code + design tour

- [`src/ai_minerals/aoi.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/aoi.py): AOI and CRS constants.
- [`src/ai_minerals/grid.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/grid.py): 500 m EPSG:3338 grid.
- [`src/ai_minerals/features/`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/features/): labels, rasters, geology, geochem, assemble.
- [`src/ai_minerals/model.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/model.py): pseudo-negatives, spatial block CV, LR baseline, success-rate curve.
- [`src/ai_minerals/model_rf.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/model_rf.py): RF / HGB factories, tree spatial CV, count-feature helper.
- [`src/ai_minerals/model_pu.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/model_pu.py): Mordelet-&-Vert PU bagging with OOB averaging.
- [`docs/mvp-proposal.md`](https://github.com/00skyking00/ai-minerals/blob/main/docs/mvp-proposal.md): scoping document with v1 / v1.1 trace.
