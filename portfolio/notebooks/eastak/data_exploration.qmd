---
title: "Eastern Alaska: data exploration"
subtitle: "A tour of the v1 datasets, region, and exploration problem"
format:
  html:
    toc: true
    code-fold: show
...
locations by their likelihood of hosting a mineral deposit.
...
This AOI straddles the tectonic boundary between the **Wrangellia** and
**Yukon-Tanana** terranes.
It's an established porphyry belt with 56 porphyry-family occurrences
in the USGS Alaska Resource Data File (ARDF) and several active
...
([`src/ai_minerals/features/assemble.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/features/assemble.py))
...
quadrangles (Tanacross, Mt Hayes, Nabesna) that span the boundary between
the Wrangellia and Yukon-Tanana terranes. We want to build a model that
...
up from 15 in Tanacross alone. That jump is the main reason we expanded the
AOI. The codes cover the canonical porphyry Cu-Mo (21a) plus three
genetically related sub-types:
...
Multi-decade, multi-agency legacy data is exactly what the industry calls
"**dark data**" when it's scanned reports, and "ingested data" when it's
already standardized like AGDB4 is. This dataset is unusually clean for
mineral exploration. The USGS has already done the painful integration of
samples analyzed by 120 different lab methods into one "best-value" schema with
provenance kept.
...
4. **Unfixable**: Rainbow Ridge (MH quadrangle) is a 15 × 15 km patch
   ...
   points, and the chapters flag that limitation where it bites.
...
3. **The data are messy but well-documented.** Legacy samples from four
   decades, multiple agencies, multiple analytical methods, with provenance
   kept for each. AGDB4's "best value" schema gets us most of the way; USGS
   has already done the painful harmonization.
4. **The AOI has structure.** Porphyry sites cluster geographically:
   along specific geologic units and sub-regions of the boundary between
   the Wrangellia and Yukon-Tanana terranes. A model that respects spatial
