...
The architectural payoff of the v2 plan: **adding a second region with a different jurisdiction's data conventions took ~600 LOC of adapters + fetchers** — all downstream modeling code (spatial CV, SHAP, PU bagging, feature engineering) worked unchanged via the canonical-schema adapter layer. See [`src/ai_minerals/data/adapters/`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/data/adapters/).
...
- **v1.2**: prospect-scale features (ASTER SWIR), physics-consistent inversions, end-to-end PU learning. Decision-theoretic drill planning has now been built — see [`decision_planning.qmd`](decision_planning.qmd).
...
All layers reprojected to **EPSG:3005 (BC Albers Equal Area)** and aggregated to a 500 m grid. Adapters in [`src/ai_minerals/data/adapters/`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/data/adapters/) normalize each source to a canonical internal schema so the feature pipeline is jurisdiction-agnostic.
...
1. **Decision-theoretic drill planning** — *built and linked: see [`decision_planning.qmd`](decision_planning.qmd).* POMDP over the model's posterior on a 30×30 cell subarea, with random / greedy / POMCP baselines. v1.0-fidelity (Mern et al.) framing; honest about the independent-Bernoulli + noiseless-sensor simplifications that limit POMCP's lift over greedy in this particular problem setup.
2. **ASTER SWIR alteration mapping** — 30 m SWIR, six narrow bands; resolves specific alteration minerals (kaolinite, alunite, muscovite, chlorite) that S2 can only see as mixtures.
3. **Physics-consistent geophysical inversions** (SimPEG) of the NRCan magnetic + gravity to produce subsurface susceptibility/density rather than surface-grid anomalies.
4. **End-to-end PU learning** with weighted-PU or Elkan-Noto calibration, not just post-hoc ranking comparison.
5. **Extend the AOI north** to Yukon — brings in the Carmacks / Minto porphyry belt, reuses every piece of infrastructure.

## Code + design tour

- [`src/ai_minerals/regions/bcgt.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/regions/bcgt.py) — region config (AOI, CRS, deposit classes, pathfinder elements, raw-file paths).
- [`src/ai_minerals/data/adapters/`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/data/adapters/) — canonical-schema adapters. New BC entries: `occurrences/bc_minfile.py`, `geochem/bcgs_rgs.py`, `geology/bcgs_digital.py`, `drillholes/bcgs_geofile.py`.
- [`src/ai_minerals/data/bcgs_*.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/data/) + [`gsc_geophysics.py`](https://github.com/00skyking00/ai-minerals/blob/main/src/ai_minerals/data/gsc_geophysics.py) — BC/NRCan fetchers.
