...
1. **Decision-theoretic drill planning.** *Built and linked: see [`decision_planning.qmd`](decision_planning.qmd).* POMDP over the model's posterior on a 30×30 cell subarea, with random / greedy / POMCP baselines. v1.0-fidelity (Mern et al.) framing; flags the independent-Bernoulli + noiseless-sensor simplifications that limit POMCP's lift over greedy in this particular problem setup.
