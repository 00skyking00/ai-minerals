...
#
# Usage:
#   bash scripts/v37_post_training_runbook.sh        # default: full pipeline
#   bash scripts/v37_post_training_runbook.sh --skip-audit-suite  # skip the
#                                                                  # 4-6 hour
#                                                                  # F.2 suite
#
...
$PY scripts/northern_sierra_placer_calibrate_and_fuse.py --no-logistic-fusion

# --- 4. Refresh sidecars ---
section "4a. Coverage-mask sidecar"
$PY scripts/build_placer_coverage_mask.py

section "4b. Bands/calibration JSON sidecar"
$PY scripts/build_placer_bands_json.py

section "4c. 2-band GeoTIFF"
$PY scripts/build_placer_2band.py
...
$PY scripts/v37_southern_anchor_held_out.py

# --- 6. F.2 audit suite ---
if [ "$RUN_AUDIT_SUITE" = "true" ]; then
  section "6. F.2 audit suite (4-6 hours; will run unattended)"
  bash scripts/northern_sierra_placer_audit_suite.sh
else
  echo
