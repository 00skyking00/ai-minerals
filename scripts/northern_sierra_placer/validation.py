...
Usage:
    .venv/bin/python scripts/northern_sierra_placer/validation.py
"""
...
def _format_recipe_section(recipes: dict[str, dict]) -> list[str]:
    ...
    if not recipes:
        lines.append("_No `recipe_match_<pop>.json` files present; run "
                     "`scripts/northern_sierra_placer/rationale_250m.py` first._")
        return lines
...
def _render_model_card(
    *,
    ...
    push("- Phase 2 training: `scripts/northern_sierra_placer/train_predict_250m.py`")
    push("- Fusion: `scripts/northern_sierra_placer/calibrate_and_fuse.py`")
    push("- This validation script: `scripts/northern_sierra_placer/validation.py`")
