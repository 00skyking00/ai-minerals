...
`scripts/northern_sierra_placer/fetch_osm_rivers.py`; entries are
...
Usage (from the repo root, so `./.osm_cache` lands where osmnx looks):
    .venv/bin/python scripts/northern_sierra_placer/fetch_osm_mining.py
...
(`scripts/northern_sierra_placer/assemble_250m.py --augment-osm-mining`);
