...
Usage:
    .venv/bin/python scripts/northern_sierra_placer/calibrate_and_fuse.py
    .venv/bin/python scripts/northern_sierra_placer/calibrate_and_fuse.py --no-logistic-fusion
"""
...
def _load(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            f"{path} missing. Run scripts/northern_sierra_placer/train_predict_250m.py "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if t_missing and q_missing:
        ...
        print(f"    Run scripts/northern_sierra_placer/train_predict_250m.py first.")
        return 1
